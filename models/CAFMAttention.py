...
class MSFN(nn.Module):
    def __init__(self, dim, ffn_expansion_factor=2.66, bias=False):
        ...
        self.project_in = nn.Conv2d(dim, hidden_features * 3, kernel_size=(1, 1, 1), bias=bias)

        self.dwconv1 = nn.Conv2d(hidden_features, hidden_features, kernel_size=(3, 3, 3), stride=1, dilation=1,
        ...
        self.project_out = nn.Conv2d(hidden_features, dim, kernel_size=(1, 1, 1), bias=bias)
...
class CAFMAttention(nn.Module):
    def __init__(self, dim, num_heads=2, bias=False):
        ...
        self.qkv = nn.Conv2d(dim, dim * 3, kernel_size=(1, 1, 1), bias=bias)
        self.qkv_dwconv = nn.Conv2d(dim * 3, dim * 3, kernel_size=(3, 3, 3), stride=1, padding=1, groups=dim * 3,
                                    bias=bias)
        self.project_out = nn.Conv2d(dim, dim, kernel_size=(1, 1, 1), bias=bias)
        self.fc = nn.Conv2d(3 * self.num_heads, 9, kernel_size=(1, 1, 1), bias=True)

        self.dep_conv = nn.Conv2d(9 * dim // self.num_heads, dim, kernel_size=(3, 3, 3), bias=True,
