...
def wasserstein_loss(pred, target, eps=1e-7, constant=12.8):  # 计算归一化的Wasserstein距离
    ...
    def __call__(self, p, targets):  # predictions, targets
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                iou_ratio = 0.5  # 设置nwd和Iou损失所在的权重，如果Iou_ratio为0，那么计算最终结果只会考虑nwd损失，而不会考虑IOU
                lbox += (1 - iou_ratio) * (1.0 - nwd).mean() + iou_ratio * (1.0 - iou).mean()  # iou loss

                # Objectness   clamp：值域的限定
